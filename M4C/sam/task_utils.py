...
def get_loader(task_cfg, tokenizer, split):
    ...
    dataset_names = task_cfg[f"{split}_on"] # a list of dataset name from config file (textvqa/stvqa/ both)
    assert isinstance(dataset_names, list)

    datasets = [] # contains actual dataset objects (TextVQADataset or STVQADataset or both)
    for dset in dataset_names:
        _dataset = DatasetMapTrain[dset]( # DatasetMapTrain - a dict for maping dataset class with dataset name
    ...
    random_sampler = RandomSampler(dataset_instance) # Torch sampler
    loader = DataLoader(
        dataset_instance,
