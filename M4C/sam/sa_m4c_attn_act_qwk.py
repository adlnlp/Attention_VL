...
#from pytorch_transformers.modeling_bert import (BertConfig, BertEmbeddings,BertEncoder, BertLayerNorm,BertPreTrainedModel)
...
class SAM4CAttnActQWK(nn.Module):
    """
...
class SpatialBertSelfAttention(nn.Module):
    ...
    def __init__(self, config, use_implicit=False):
        ...
        # nope, nothing, no implicit relations
    ...
    def transpose_for_scores(self, x):
        ...
        ) # bs, num_feats, num_attention_heads, attention_head_size
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3) # bs, num_attention_heads, num_feats, attention_head_sie
    ...
    def forward(
        ...
        # flip and have invalid pairs being -10000 - additional bias term in the compatibility function to compute attention score
        spatial_attention_mask = (1.0 - spatial_attention_mask) * -10000.0
        spatial_attention_mask = spatial_attention_mask.permute((0, 3, 1, 2))  # bs, num_attention_heads, num_feats, num_feats
...
class SpatialBertAttention(nn.Module):
    ...
    # ???? didnt use this function at all?? what's this for
    def prune_heads(self, heads):
        ...
        # Prune linear layers
        self.self.query = prune_linear_layer(self.self.query, index) # ???? where did they define the prune_linear_layer fn?
...
class BertSpatialEncoder(nn.Module):
    def __init__(self, config):
        ...
        from pytorchtransformers.modeling_bert_act_general_qwk import BertLayer#,BertLayer_attn
    ...
    def forward(self, hidden_states, attention_mask, batch_dict, head_mask=None):
        ...
        for layer_type, mix_type in zip(self.layer_type_list, self.mix_list):
            ...
            if layer_type == "n":
                ...
                spatial_adj_matrix = batch_dict["spatial_adj_matrices"][matrix_type] # batch_dict["spatial_adj_matrices"]["3"]
        ...
        #assert next(normal_iter_attention, None) is None
