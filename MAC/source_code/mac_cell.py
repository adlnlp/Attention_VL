...
class MACCell(tf.nn.rnn_cell.RNNCell):
    ...
    def control(self, controlInput, inWords, outWords, questionLengths,
        ...
        with tf.variable_scope("control" + name, reuse = reuse):
            ...
            if config.controlFeedPrev: #feed previous control state"
                newContControl = control if config.controlFeedPrevAtt else contControl #feed previous control post word attention (otherwise will feed continuous control)
                if config.controlFeedInputs:#feed question representation"
                    newContControl = tf.concat([newContControl, controlInput], axis = -1)
                    dim += config.ctrlDim
                ##--> newContControl:[c_i-1:q_i]
            ...
            ##################### change here###########################
            newContControl = tf.expand_dims(newContControl, axis = 1) #[batch,1,ctrl_dim]
            # original - element wise multiply
            if config.control_element_wise_multiply:
                print('control attention: original, element-wise multiply')
                interactions = newContControl* inWords
                # --> [batchSize, 1, ctrlDim]*[batchSize, questionLength, ctrlDim] =[batch_size, questionLength,512]
                # compute attention distribution over words and summarize them accordingly 
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
                #--> [batch, 1, questionlen]
                # self.interL = (interW, interb)

            # general_kwq
            if config.control_general_kwq:
                print('control attention:  general KWQ')
                Wk = ops.linear(inWords,dim,config.ctrlDim,addBias = False) # [batch,question_len,ctrl_dim]
                interactions = newContControl*Wk
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
            #Biased General K(WQ+b)
            if config.control_biased_general_kwq:
                print('control attention: biased general K(WQ+b)')
                WQ = ops.linear(inWords,dim,config.ctrlDim,addBias = True)
                interactions = newContControl*WQ
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
            #ACT General K(WQ+b)
            if config.control_act_general_relu_kwq:
                print('control attention: act general relu(K(WQ+b))')
                WQ = ops.linear(inWords,dim,config.ctrlDim,addBias = True)
                interactions = newContControl*WQ
                interactions = tf.nn.relu(interactions)
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
            if config.control_act_general_relu_qwk:
                print('control attention: act general relu(Q(WK+b))')
                WK = ops.linear(newContControl,dim,config.ctrlDim,addBias = True)
                interactions = inWords*WK
                interactions = tf.nn.relu(interactions)
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")

            # scaled element wise multiply
            if config.control_scaled:
                print('control attention: scaled element-wise multiply') 
                import math
                interactions = (newContControl * inWords)/math.sqrt(dim)
                # --> [batchSize, 1, ctrlDim]*[batchSize, questionLength, ctrlDim] =[batch_size, questionLength,512]
                # compute attention distribution over words and summarize them accordingly 
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
                #--> [batch, 1, questionlen]
            ...
            # genral QWK
            if config.control_general_qwk:
                print('control attention:  general QWK')
                WK = ops.linear(newContControl,dim,config.ctrlDim,addBias = False)
                interactions = inWords*WK
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
            #Biased General Q(WK+b)
            if config.control_biased_general_qwk:
                print('control attention: biased general Q(WK+b)')
                WK = ops.linear(newContControl,dim,config.ctrlDim,addBias = True)
                interactions = inWords*WK
                logits = ops.inter2logits(interactions, dim,sumMod = "LIN")
            ...
            if config.controlConcatWords: #concatenate words to interaction when computing attention
                print('controlConcatWords true')
                interactions = tf.concat([interactions, inWords], axis = -1)
                dim += config.ctrlDim                                              

            # optional projection
            if config.controlProj:#apply linear projection on words interactions
                print('controlProj true')
                interactions = ops.linear(interactions, dim, config.ctrlDim, 
                    act = config.controlProjAct) #activation for control interactions
                dim = config.ctrlDim
    ...
    def read(self, knowledgeBase, memory, control, name = "", reuse = None):
        with tf.variable_scope("read" + name, reuse = reuse):
            ...
            #-->[I_ihw,k_hw]  #config.readMemAttType: MUL
            #-->interactions: [batchSize, kbSize , memDim]
            #-->interDim:512+
            ...
            if config.readMemProj:#project interactions with memory
                interactions = ops.linear(interactions, interDim, dim, act = config.readMemAct, 
                    name = "memKbProj") 
            else: 
                dim = interDim
            #-->I'_ihw [batch, HW, 512]
            print('control in read',tf.shape(control)) #should be 2 dim
            ## Step 2: compute interactions with control
            if config.readCtrl: #compare KB-memory interactions to control
                
                if config.ctrlDim != dim:
                    control = ops.linear(control, ctrlDim, dim, name = "ctrlProj")
                ###############################attention here######################################
                # interactions: [batchSize, kbSize, dim]
                #control: [batchSize, dim]

                # original 
                if config.read_original:
                    interactions, interDim = ops.mul(interactions, control, dim, 
                        ...
                        name = "ctrlInter")
                    # -->c_i * I' [batchSize, kbSize, dim*2]

                # scaled 
                if config.read_scaled:
                    print('read attention: scaled ')
                    interactions, interDim = ops.mul(interactions, control, dim, attn ="SCALED",
                        interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                    # -->c_i * I' [batchSize, kbSize, dim*2]


                if config.read_general_kwq:
                    print('read attention: general_kwq ')
                    interactions, interDim = ops.mul(interactions, control, dim, attn ="GENERAL_KWQ",
                        interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                   

                if config.read_biased_general_kwq:
                    print('read attention: biased_general_kwq ')
                    interactions, interDim = ops.mul(interactions, control, dim, attn ="BIASED_GENERAL_KWQ",
                        interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                    print('interactions',tf.shape(interactions)) # 3dim
                    
                if config.read_act_general_relu_kwq:
                    print('read attention: act_general_relu_kwq ')
                    interactions, interDim = ops.mul(interactions, control, dim, attn ="BIASED_GENERAL_KWQ",
                        interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                    interactions = tf.nn.relu(interactions)
                if config.read_act_general_relu_qwk:
                    print('read attention: act_general_relu_qwk ')
                    interactions, interDim = ops.mul(control,interactions, dim, attn ="BIASED_GENERAL_KWQ",
                        extendY = False,interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                    interactions = tf.nn.relu(interactions)

                if config.read_biased_general_qwk:
                    print('read attention: biased_general_qwk ')
                    interactions, interDim = ops.mul(control,interactions, dim, attn ="BIASED_GENERAL_KWQ",
                        extendY = False,interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                    print('interactions',tf.shape(interactions)) #now 4 dim ;should be 3 d
                    
          
                if config.read_general_qwk:
                    print('read attention: general_qwk ')
                    interactions, interDim = ops.mul(control,interactions, dim, attn ="GENERAL_KWQ",
                        extendY = False,interMod = config.readCtrlAttType, concat = {"x": config.readCtrlConcatInter}, 
                        name = "ctrlInter")
                ...
                if config.readCtrlConcatKB:
                    if config.readCtrlConcatProj:#concatenate projected values instead or original to control interaction
            ...
            attention = ops.inter2att(interactions, dim, "LIN",dropout = self.dropouts["read"])
                        #--> ri
            ...
            print('information',tf.shape(information)) # should be 2 dim
