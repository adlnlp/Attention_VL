...
'''
sumMod = ["LIN", "SUM"]
def inter2logits(interactions, dim, sumMod="SUM", dropout = 1.0, name = "", reuse = None):
...
[batchSize, N]
'''
def inter2att(interactions, dim,sumMod, dropout = 1.0, name = "", reuse = None):
    with tf.variable_scope("inter2att" + name, reuse = reuse): 
        logits = inter2logits(interactions, dim, sumMod,dropout = dropout)
...
def linear(inp, inDim, outDim, dropout = 1.0, 
    ...
    # print('linear output shape',tf.shape(output))
    if retVars:
        return (output, (W, b))
...
def mul(x, y, dim,attn=None, dropout = 1.0, proj = None, interMod = "MUL", concat = None, mulBias = None,
    ...
    with tf.variable_scope("mul" + name, reuse = reuse):                
        ...
        if attn == "GENERAL_KWQ": #or qwk
   
            y = linear(y, dim, dim,addBias = False)
        if attn == "BIASED_GENERAL_KWQ": #or qwk

            y = linear(y, dim, dim,addBias = True)

        if extendY:
            #if attn == "BIASED_GENERAL_KWQ" or attn == "GENERAL_KWQ":
            y = tf.expand_dims(y, axis = -2)
            # broadcasting to have the same shape
            y = tf.zeros_like(x) + y
            # shape of y => 3dim
        ...
        if not extendY: # for biased_general_qwk
        ...
        if interMod == "MUL":
            if mulBias is None:
                mulBias = config.mulBias  # defual=0           
